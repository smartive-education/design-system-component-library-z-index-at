...
export const Typography: FC<TypographyProps> = ({ type, children, htmlFor }) => {
  switch (type) {
    ...
    case 'label-xl':
      return (
        <label className="font-semibold text-slate-600  text-lg md:text-2xl" htmlFor={htmlFor}>
          {children}
        </label>
      );
    case 'label-l':
      return (
        <label className="text-slate-600 font-semibold text-base md:text-xl" htmlFor={htmlFor}>
          {children}
        </label>
      );
    case 'label-m':
      return (
        <label className="text-slate-600 font-semibold tex-sm md:text-lg" htmlFor={htmlFor}>
          {children}
        </label>
      );
    case 'label-s':
      return (
        <label className="text-slate-600 font-semibold text-xs" htmlFor={htmlFor}>
          {children}
        </label>
      );
    case 'paragraph-l':
